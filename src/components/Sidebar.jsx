...
export default function Sidebar({ collapsed, onToggle }) {
  ...
  return (
    <div className={`fixed left-0 top-0 h-full bg-gradient-to-b from-slate-50 to-white border-r border-slate-200/60 shadow-lg transition-all duration-300 z-10 flex flex-col ${
      collapsed ? 'w-25' : 'w-64'
    }`}>
      {/* Header Section */}
        ...
        <div className="flex items-center space-x-3">
          {!collapsed && (
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-lg font-bold text-slate-800 tracking-tight">Tax Admin</h1>
              </div>
            </div>
          )}
        </div>
          ...
          return (
            <div key={item.name} className="mb-1">
              <Link
                ...
                onClick={item.onToggle}
              >
                <Icon className={`${collapsed ? 'w-14 h-5' : 'w-5 h-5 mr-3'} flex-shrink-0 ${
                  isActive ? 'text-white' : 'text-slate-500 group-hover:text-slate-700'
                }`} />
                {!collapsed && (
                  <span className="flex-1 font-medium tracking-wide">{item.name}</span>
                )}
                {!collapsed && item.expandable && (
                  <div 
              ...
              {!collapsed && item.expandable && item.expanded && (
                <div className="mt-1 ml-2 bg-slate-50/80 rounded-lg border border-slate-200/50 shadow-inner">
                  {item.subItems.map((subItem, index) => (
                    <div 
                      key={index} 
                      className="px-4 py-2.5 text-sm text-slate-600 hover:bg-white/80 hover:text-slate-800 cursor-pointer transition-all duration-200 first:rounded-t-lg last:rounded-b-lg font-medium capitalize tracking-wide border-b border-slate-200/30 last:border-b-0"
                      onClick={() => handleStatusClick(subItem)}
                    >
                      <span className="flex items-center">
                        <div className="w-1.5 h-1.5 bg-slate-400 rounded-full mr-3 opacity-60"></div>
                        {subItem}
                      </span>
                    </div>
                  ))}
        ...
        <div className="relative mb-3" ref={notificationRef}>
          ...
          >
            <div className="flex items-center">
              ...
              {!collapsed && <span className="ml-3 text-sm font-medium text-slate-700 tracking-wide">Notifications</span>}
            </div>
            {unreadCount > 0 && (
            ...
            {collapsed ? (
              ...
              <>
                <div className="flex items-center">
                  ...
                  <div className="text-left">
                    <div className="font-semibold text-slate-800 text-sm tracking-wide">{user?.name}</div>
                    <div className="text-xs text-slate-500 capitalize font-medium">{user?.role}</div>
                  </div>
                </div>
                <ChevronDown className={`w-4 h-4 text-slate-500 transition-all duration-200 ${showUserMenu ? 'rotate-180' : ''}`} />
              </>
            )}
          </button>

          {showUserMenu && (
            <div className={`absolute bg-white rounded-lg shadow-xl border border-slate-200/60 py-2 z-50 backdrop-blur-sm ${
              collapsed ? 'left-16 bottom-0 w-48' : 'left-0 bottom-full mb-2 w-full'
            }`}>
