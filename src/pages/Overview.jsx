...
import React, { useState, useEffect } from 'react';
import { Users, Receipt, CreditCard, TrendingUp, TrendingDown } from 'lucide-react';
...
const [activities, setActivities] = useState([]);
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const returnsByStatus = data.returns.reduce((acc, ret) => {
        const key = ret.return_status.toLowerCase();
        acc[key] = (acc[key] || 0) + 1;
        return acc;
      }, {});

      const totalInvoiceAmount = data.invoices.reduce(
        (sum, inv) => sum + parseFloat(inv.invoice_amount),
        0
      );

      const pendingInvoices = data.invoices.filter(i => i.invoice_status.toLowerCase() === 'pending').length;
      const paidInvoices = data.invoices.filter(i => i.invoice_status.toLowerCase() === 'paid').length;
      ...
      setStats({
        ...
        inReviewReturns: returnsByStatus['in review'] || 0,
        inPreparationReturns: returnsByStatus['in preparation'] || 0,
        documentVerified: returnsByStatus['documents verified'] || returnsByStatus['document verified'] || 0,
  ...
  const statCards = [
    ...
    {
      title: 'Payments Received',
      value: `$${(stats.totalInvoiceAmount || 0).toLocaleString()}`,
    ...
    <div className="space-y-6 p-6 bg-gray-50 min-h-screen">
            ...
            <div key={card.title} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  {TrendIcon && <TrendIcon className={`w-4 h-4 mt-1 ${card.trendColor}`} />}
                </div>
                <div className={`p-3 rounded-full bg-${card.color}-100`}>
                  <IconComponent className={`w-6 h-6 text-${card.color}-600`} />
                </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-indigo-600">{stats.documentVerified || 0}</div>
            <div className="text-sm text-gray-600">Document Verified</div>
          </div>
        </div>
      </div>
